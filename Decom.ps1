...
try {
    ...
    $commercialappid = Get-AzKeyVaultSecret -VaultName 'some_key_vault' -Name 'some_secret_name' -AsPlainText
    $commercialappsecret = Get-AzKeyVaultSecret -VaultName 'some_key_vault' -Name 'some_secret_name' -AsPlainText
    $commercialtenantid = Get-AzKeyVaultSecret -VaultName 'some_key_vault' -Name 'some_secret_name' -AsPlainText
    $govappid = Get-AzKeyVaultSecret -VaultName 'some_key_vault' -Name 'some_secret_name' -AsPlainText
    $govappsecret = Get-AzKeyVaultSecret -VaultName 'some_key_vault' -Name 'some_secret_name' -AsPlainText
    $govtenantid = Get-AzKeyVaultSecret -VaultName 'some_key_vault' -Name 'some_secret_name' -AsPlainText
    $gccappid = Get-AzKeyVaultSecret -VaultName 'some_key_vault' -Name 'some_secret_name' -AsPlainText
    $gccappsecret = Get-AzKeyVaultSecret -VaultName 'some_key_vault' -Name 'some_secret_name' -AsPlainText
    $gcctenantid = Get-AzKeyVaultSecret -VaultName 'some_key_vault' -Name 'some_secret_name' -AsPlainText
    $TenableaccessKey = Get-AzKeyVaultSecret -vaultName 'some_key_vault' -name 'some_secret_name' -AsPlainText
    $TenablesecretKey = Get-AzKeyVaultSecret -vaultName 'some_key_vault' -name 'some_secret_name' -AsPlainText
    $prodpass = Get-AzKeyVaultSecret -vaultName 'some_key_vault' -name 'some_secret_name' -AsPlainText 
    $sqlinstance = 'some_sql_instance'
    $sqlDatabase = 'some_sql_database'
    $SqlCredential = New-Object System.Management.Automation.PSCredential ('some_secret_name', ((Get-AzKeyVaultSecret -vaultName "some_key_vault" -name 'some_secret_name').SecretValue))
}
catch {
...
$CRmeta = "some_URL_endpoint/$($VmRF.Change_Number)"
...
$crticketendpoint = "some_URL_endpoint/$($getCRticket.result.sys_id)"
...
-Query "SELECT TOP 1 * FROM some_table_name `
        WHERE some_field = @vmchangenumber `
        AND some_field IS NOT NULL `
        AND some_field = 'Y'" -SqlParameters @{vmchangenumber = $VmRF.Change_Number}
...
# Get RITM info
$ritmmeta = "some_URL_endpoint/$($ritmnumber)"
...
# Get requestor info
$usermeta = "some_URL_endpoint/$($usersysid)"
    ...
    ($DeleteVMObject[0].Status[4] -eq 'Passed') -or ($DeleteVMObject[0].Status[4] -eq 'Skipped'))
{
    ...
    $delete_vm_url = "some_URL_endpoint/$($getCRticket.result.sys_id)"
...
if (($DeleteADObject[0].Status -eq 'Passed') -and ($DeleteADObject[1].Status -eq 'Skipped'))
{
    ...
    $delete_ADObject_url = "some_URL_endpoint/$($getCRticket.result.sys_id)"
...
if (($UnlinkVMObject[0][0].Status -eq 'Passed') -and ($UnlinkVMObject[0][1].Status -eq 'Skipped'))
{
    ...
    $tenable_object_url = "some_URL_endpoint/$($getCRticket.result.sys_id)"
...
$changeimplement = "some_URL_endpoint/$($getCRticket.result.sys_id)"
...
$changetasks ="some_URL_endpoint/$($getCRticket.result.number)^state=1"
...
{
    $changetaskendpoint ="some_URL_endpoint/$($getCRticket.result.sys_id)/$($changetask)"
...
{ 
    ...
    $changeclosed = "some_URL_endpoint/$($getCRticket.result.sys_id)"
